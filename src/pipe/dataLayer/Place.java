...
public class Place extends PlaceTransitionObject {
	 ...
	 *            Capacity
	 */
	public Place(double positionXInput, double positionYInput, String idInput, String nameInput,
			Double nameOffsetXInput, Double nameOffsetYInput, int initialMarkingInput,
	...
	public Place copyPlace() {
		Place newPlace = new Place(this.positionX, this.positionY);
		newPlace.setName(this.getName());
		abToken oldabToken = this.getToken();
		abToken newabToken = new abToken();
		newabToken.definetype(oldabToken.getDataType());
		for (int i = 0; i < oldabToken.getTokenCount(); i++) {
			Token oldToken = oldabToken.getTokenbyIndex(i);
			Token newToken = new Token();
			BasicType[] bt = new BasicType[oldToken.getBTsize()];
			for (int j = 0; j < oldToken.getBTsize(); j++) {
				BasicType newBasicType = new BasicType();
				newBasicType.kind = oldToken.getBTbyindex(j).kind;
				newBasicType.Tint = oldToken.getBTbyindex(j).Tint;
				newBasicType.Tstring = oldToken.getBTbyindex(j).Tstring;
				bt[j] = newBasicType;
			}
			newToken.add(bt);
			newToken.defineTlist(oldabToken.getDataType());
			newabToken.addToken(newToken);
		}
		newPlace.setToken(newabToken);
		return newPlace;
	}
	...
	public Place copy() {
		Place copy = new Place(Zoomer.getUnzoomedValue(this.getX(), zoom), Zoomer.getUnzoomedValue(
				this.getY(), zoom));
	...
	public boolean contains(int x, int y) {
		...
		if (someArc != null) { // Must be drawing a new Arc if non-NULL.
			if ((proximityPlace.contains((int) unZoomedX, (int) unZoomedY) || place.contains(
					(int) unZoomedX, (int) unZoomedY)) && areNotSameType(someArc.getSource())) {
	...
	public void updateEndPoint(Arc arc) {
		if (arc.getSource() == this) {
			...
			arc.setSourceLocation(
					positionX + centreOffsetLeft() - (0.5 * getDiameter() * (Math.sin(angle))),
					positionY + centreOffsetTop() + (0.5 * getDiameter() * (Math.cos(angle))));
		} else {
			// Make it calculate the angle from the centre of the place rather
	...
	public void showEditor() {
		...
		// 2 Add Place editor
		contentPane.add(new PlaceEditorPanel(guiDialog.getRootPane(), this, CreateGui.getModel(),
				CreateGui.getView()));
	...
	public void showType() {
		...
		// 2 Add Place editor
		contentPane.add(new PlaceTypePanel(guiDialog.getRootPane(), this, group, CreateGui
				.getModel(), CreateGui.getView()));
	...
	public UndoableEdit changeMarkingParameter(MarkingParameter _markingParameter) {
		...
		currentMarking = markingParameter.getValue();
		update();
		return new ChangeMarkingParameterEdit(this, oldMarkingParameter, markingParameter);
	}

...
public class GuiFrame extends JFrame implements ActionListener, Observer {
	...
	private final String[] zoomExamples = { "40%", "60%", "80%", "100%", "120%", "140%", "160%",
			"180%", "200%", "300%" };
	...
	private FileAction createAction, openAction, closeAction, saveAction, saveAsAction, exitAction,
			printAction, exportPNGAction, exportTNAction, exportPSAction, importAction;
	...
	private TypeAction annotationAction, arcAction, inhibarcAction, placeAction, transAction,
			timedtransAction, tokenAction, selectAction, rateAction, markingAction,
			deleteTokenAction, dragAction;
	private AnimateAction startAction, stepforwardAction, stepbackwardAction, randomLowLevelAction,
			randomHighLevelAction, undoFire, randomAnimateAction, modelCheckingAction;
	...
	private void buildMenus() {
		...
		addMenuItem(fileMenu, createAction = new FileAction("New", "Create a new Petri net",
				"ctrl N"));
		addMenuItem(fileMenu, openAction = new FileAction("Open", "Open", "ctrl O"));
		addMenuItem(fileMenu, closeAction = new FileAction("Close", "Close the current tab",
				"ctrl W"));
		...
		addMenuItem(editMenu, deleteAction = new DeleteAction("Delete", "Delete selection",
				"DELETE"));

		JMenu drawMenu = new JMenu("Draw");
		drawMenu.setMnemonic('D');
		addMenuItem(drawMenu, selectAction = new TypeAction("Select", Pipe.SELECT,
				"Select components", "S", true));
		drawMenu.addSeparator();
		addMenuItem(drawMenu, placeAction = new TypeAction("Place", Pipe.PLACE, "Add a place", "P",
				true));
		addMenuItem(drawMenu, transAction = new TypeAction("Immediate transition", Pipe.IMMTRANS,
				"Add an immediate transition", "I", true));
		addMenuItem(drawMenu, timedtransAction = new TypeAction("Timed transition",
				Pipe.TIMEDTRANS, "Add a timed transition", "T", true));
		...
		drawMenu.addSeparator();
		addMenuItem(drawMenu, tokenAction = new TypeAction("Add token", Pipe.ADDTOKEN,
				"Add a token", "ADD", true));
		addMenuItem(drawMenu, deleteTokenAction = new TypeAction("Delete token", Pipe.DELTOKEN,
				"Delete a token", "SUBTRACT", true));
		drawMenu.addSeparator();
		addMenuItem(drawMenu, rateAction = new TypeAction("Rate Parameter", Pipe.RATE,
				"Rate Parameter", "R", true));
		addMenuItem(drawMenu, markingAction = new TypeAction("Marking Parameter", Pipe.MARKING,
				"Marking Parameter", "M", true));
		...
		addMenuItem(viewMenu, zoomOutAction = new ZoomAction("Zoom out", "Zoom out by 10% ",
				"ctrl MINUS"));
		addMenuItem(viewMenu, zoomInAction = new ZoomAction("Zoom in", "Zoom in by 10% ",
				"ctrl PLUS"));
		...
		addMenuItem(viewMenu, dragAction = new TypeAction("Drag", Pipe.DRAG, "Drag the drawing",
				"D", true));
		...
		addMenuItem(animateMenu, randomLowLevelAction = new AnimateAction("RandomLowLevel",
				Pipe.RANDOMLOWLEVEL, "Randomly fire a low level transition", "5"));
		addMenuItem(animateMenu, randomHighLevelAction = new AnimateAction("RandomHighLevel",
				Pipe.RANDOMHIGHLEVEL, "Randomly fire a high level transition", "6"));
		...
		addMenuItem(animateMenu, modelCheckingAction = new AnimateAction("ModelChecking",
				Pipe.MODELCHECKING, "Model checking Petri nets", "8", true));
		addMenuItem(animateMenu, undoFire = new AnimateAction("UndoFire", Pipe.UNDOFIRE,
				"Undo Fire", "9"));
		...
		addMenuItem(experimentMenu, loadExperimentAction = new ExperimentAction("Load experiment",
				"Load an experiment file", ""));
		addMenuItem(experimentMenu, experimentEditorAction = new ExperimentAction(
	...
	private void buildToolbar() {
		...
		addZoomComboBox(toolBar, new ZoomAction("Zoom", "Select zoom percentage ", ""));
		...
		addButton(animationToolBar, undoFire);
		...
		JOptionPane
				.showMessageDialog(
						...
						"Imperial College DoC MSc Group And MSc Individual Project\n\n"
								...
								+ "http://pipe2.sourceforge.net/", "About PIPE2",
	...
	private void saveNet(File outFile) {
		try {
			...
			JOptionPane.showMessageDialog(GuiFrame.this, e.toString(), "File Output Error",
	...
	private void loadExperiment(String path) {
		...
		try {
			...
			JOptionPane.showMessageDialog(GuiFrame.this, "The Experiment file is not valid."
	...
	private boolean checkForSave() {
		...
		if (appView.getNetChanged()) {
			int result = JOptionPane.showConfirmDialog(GuiFrame.this,
					"Current file has changed. Save current file?", "Confirm Save Current File",
	...
	public void updateZoomCombo() {
		...
		zoomComboBox
				.setSelectedItem(String.valueOf(appView.getZoomController().getPercent()) + "%");
	...
	class AnimateAction extends GuiAction {
		...
		public void actionPerformed(ActionEvent ae) {
			...
			case Pipe.MODELCHECKING:
				animBox.clearStepsForward();
				AnalysisModuleHandler mcHandler = new AnalysisModuleHandler();
			...
			case Pipe.UNDOFIRE:
				animBox.clearStepsForward();
				CreateGui.getAnimator().undoFire();
				stepforwardAction.setEnabled(animBox.isStepForwardAllowed());
				stepbackwardAction.setEnabled(animBox.isStepBackAllowed());
				break;
	...
	class ExampleFileAction extends GuiAction {
		...
		ExampleFileAction(File file, String keyStroke) {
			...
			putValue(SMALL_ICON, new ImageIcon(Thread.currentThread().getContextClassLoader()
		...
		ExampleFileAction(JarEntry entry, String keyStroke) {
			super(entry.getName().substring(
					1 + entry.getName().indexOf(System.getProperty("file.separator"))),
